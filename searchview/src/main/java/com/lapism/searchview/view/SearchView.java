...
public class SearchView extends FrameLayout implements Filter.FilterListener, View.OnClickListener {
    ...
    public void setVersion(int version) {
        ...
        if (mVersion == SearchCodes.VERSION_TOOLBAR) {
            ...
            params.setMargins(leftStart, 0, rightEnd, bottom);
        }
...
//            mRecyclerView.animate().alpha(1.0f);
    ...
    public void onClick(View v) {
        ...
        if (v == mEmptyImageView) {
            ...
            if (mOnQueryChangeListener != null)
                mOnQueryChangeListener.onQueryTextSubmit(null);
        }
    }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (mSavedState.isSearchOpen) {
            show(true);
            mEditText.setText(mSavedState.query);
            if (mSavedState.query != null) {
                mEditText.setSelection(mEditText.length());
                mUserQuery = mSavedState.query;
            }
            if (TextUtils.isEmpty(mSavedState.query))
                out();
        }
